...
class DFM_PL(BinaryClassification):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        wave_real = batch['audio']
        labels = batch['label'] if "label" in batch else torch.ones(wave_real.size(0), device=wave_real.device)
        
        # 1. 获取 Domain Labels 并映射阈值
        if 'domain_label' in batch:
            domain_labels = batch['domain_label'].to(self.device) # (B,)
        else:
            # 默认为 0 (Libri)
            domain_labels = torch.zeros(wave_real.size(0), dtype=torch.long, device=self.device)
        
        # 查表得到每个样本的能量阈值 (B,)
        energy_thresholds = self.energy_threshold_map[domain_labels]
        ...
        if stage_name == "stage1":
            ...
            # 获取 Real 数据
            emb_real = batch_res["emb"]            
            # 计算能量 Mask
            expected_len = emb_real.shape[1]
            mask_real = self.get_energy_mask(wave_real, target_len=expected_len, threshold=energy_thresholds).to(self.device)
            ...
            with torch.no_grad():
                wave_fake_gen, mask_wave = self.asv2021_mlaad_augmenter(wave_real.clone())
                wave_fake_gen, mask_wave = wave_fake_gen.to(self.device), mask_wave.to(self.device)

            # 1. 提取特征 (B, T, D)
            emb_fake = self.model(wave_fake_gen.squeeze(0), target_layer=self.hparams.target_layer)
            B_fake, T_fake, D_fake = emb_fake.shape
            
            # 2. 对齐 Mask
            expected_len = emb_fake.shape[1]
            mask_patch = F.adaptive_max_pool1d(mask_wave, output_size=expected_len).squeeze(1) # Augmentation Mask
            
            # 【关键】使用对应的能量阈值
            eng_mask = self.get_energy_mask(wave_fake_gen.squeeze(0), target_len=expected_len, threshold=0.01).to(self.device)
            
            # 有效 Mask = 被篡改 AND 有能量
            valid_mask = mask_patch * eng_mask 
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # 1. 获取 Domain Label 用于 VAD (如果存在)
        # 如果验证集没有 domain_label，可以使用默认值 0.03
        if 'domain_label' in batch:
            domain_labels = batch['domain_label'].to(self.device)
            energy_thresholds = self.energy_threshold_map[domain_labels]
        else:
            # 全局保守阈值
            energy_thresholds = torch.ones(wave.size(0), device=self.device) * 0.01
        ...
        eng_mask = self.get_energy_mask(wave.squeeze(1), target_len=expected_len, threshold=energy_thresholds).to(self.device)
    ...
    def get_energy_mask(self, wave, target_len, threshold):
        """
        计算音频能量掩码。
        threshold: 可以是 float，也可以是 (B,) 的 Tensor
        """
        B, T_wave = wave.shape
        stride = T_wave // target_len
