...
class ASV2021_AudioDs(AudioDataset):
    ...
    def _read_metadata_2019(self, root_path, *args, **kwargs):
    ...
    def get_splits(self):
        ...
        # data = sub_datas[-1]
        # real_data = data.query(f"vocoder == '{VOCODERs[0]}'").reset_index(drop=True)
        # inner_data = data.query(
        #     "attack in ['A07', 'A08', 'A09', 'A10', 'A11', 'A12', 'A13', 'A14', 'A15', 'A16', 'A17', 'A18', 'A19']"
        # ).reset_index(drop=True)
        # ids = set(inner_data.index)
        # other_index = [x for x in data.index if x not in ids]
        data2 = self.get_test_splits()
    ...
    def get_test_splits(self, data=None):
        ...
        return sub_datas  # 0: AR, 1:NAR, 2:TRD, 3:UKN, 4: waveform_concatenation
