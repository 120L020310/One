...
def generate_metadata(root_path):
    ...
    for file in read_file_paths_from_folder(root_path, exts="txt"):
        ...
        txt_filename = os.path.split(file)[1].split(".")[
            0
        ]  ### train, val, A1, ..., C1, C2, ....
        ...
        _data["split"] = _data["filename"].map(
            lambda x: (
                txt_filename
                if txt_filename in ["train", "val"]
                else f"test-{txt_filename}"
            )
        )
        ...
        _data["vocoder"] = _data.apply(
            lambda x: (
                ...
                if x["label"] == 1
                    ...
                    "C" + str(x["method"])
                    if str(x["method"]) in ["1", "2", "3", "4", "5", "6", "7"]
                    else x["method"]
                )
            ),
            axis=1,
...
def read_wav_paths(root_path):
    ...
    data["relative_path"] = data["audio_path"].map(
        lambda x: x.split(f"{root_path}/")[1]
    )
