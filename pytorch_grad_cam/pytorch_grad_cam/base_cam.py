...
class BaseCAM:
    ...
    def get_cam_image(
        ...
        weights = self.get_cam_weights(self.target_spec, target_layer, targets, activations, grads)
    ...
    def forward(
        ...
        self.target_spec = self.model.feature_model2D.preprocess(input_tensor)
        if self.compute_input_gradient:
            input_tensor = torch.autograd.Variable(input_tensor, requires_grad=True)

        self.outputs = outputs = self.activations_and_grads(input_tensor)
        logit = torch.sigmoid(outputs)[:, None]  # (B, 1)
        logit = torch.concat([1 - logit, logit], dim=-1)  # (B, 2)
        self.outputs = outputs = logit
        if targets is None:
        ...
        if self.uses_gradients:
            ...
            if self.detach:
                ...
                grad_min = self.activations_and_grads.gradients[0].min()
                grad_max = self.activations_and_grads.gradients[0].max()
                self.activations_and_grads.gradients[0]= (self.activations_and_grads.gradients[0] - grad_min) / (grad_max - grad_min)
            else:
    ...
    def compute_cam_per_layer(
        ...
        target_size = (self.get_target_width_height(self.target_spec))
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = self.get_cam_image(self.target_spec, target_layer, targets, layer_activations, layer_grads, eigen_smooth)
