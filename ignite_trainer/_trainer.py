    ...
    with _utils.tqdm_stdout(orig_stdout) as orig_stdout:
        ...
        if hasattr(train_loader.dataset, "class_weights"):
            model.register_buffer(
                "class_weights",
                train_loader.dataset.class_weights.clone().exp(),
                persistent=False,
            )
        if hasattr(train_loader.dataset, "label_to_class_idx"):
            model.label_to_class_idx = {
                idx: lb for idx, lb in train_loader.dataset.label_to_class_idx.items()
            }
        ...
        param_groups.append(
            {
                "params": [p for p in model.module.audio.fbsp.parameters()]
                + [model.module.logit_scale_ai, model.module.logit_scale_at],
                "weight_decay": 0.0,
            }
        )
        ###########################          给模型分配权重           #####################################################
        ...
        if scheduler_class is not None:
            ...
            scheduler: Optional[torch.optim.lr_scheduler._LRScheduler] = Scheduler(
        ...
        num_params_train = sum(
            p.numel() for grp in optimizer.param_groups for p in grp["params"]
        )
        ...
        tqdm.tqdm.write(f"\n{Network.__name__}\n")
        tqdm.tqdm.write(
            "Total number of parameters: {:.2f}{}".format(
                num_params_total, params_total_label
            )
        )
        tqdm.tqdm.write(
            "Number of trainable parameters: {:.2f}{}".format(
                num_params_train, params_train_label
            )
        )
        ...
        def training_step(engine: ieng.Engine, batch) -> torch.Tensor:
            ...
            batch_indices = torch.arange(
                audio.shape[0], dtype=torch.int64, device=device
            )
        ...
        def eval_step(_: ieng.Engine, batch) -> _interfaces.TensorPair:
            ...
            with torch.no_grad():
                ...
                ((_, _, text_features), _), _ = model(
                    text=[
                        ...
                        for class_idx in sorted(
                            eval_loader.dataset.class_idx_to_label.keys()
                        )
                    ],
                    batch_indices=torch.arange(
                        len(eval_loader.dataset.class_idx_to_label),
                        dtype=torch.int64,
                        device=device,
                    ),
                )
                ...
                logit_scale_at = torch.clamp(
                    model.module.logit_scale_at.exp(), min=1.0, max=100.0
                )
                y_pred = (
                    logit_scale_at * audio_features @ text_features.transpose(-1, -2)
                ).squeeze(1)
                ...
                for item_idx, labels in enumerate(text):
                    class_ids = list(
                        sorted(
                            [
                                eval_loader.dataset.label_to_class_idx[lb]
                                for lb in labels
                            ]
                        )
                    )
        ...
        default_metrics = {
            "Loss": {
                ...
                "y_label": (
                    ...
                    else model.module.loss_fn_name
                ),
                "width": 940,
                "height": 416,
                "lines": [
        ...
        for scope_name, scope in performance_metrics.items():
            ...
            for line in scope["lines"]:
                if "object" not in line:
                    line["object"]: imet.Metric = _utils.load_class(line["class"])(
                        **line["args"]
                    )

                line["metric_label"] = "{}: {}".format(
                    scope["window_name"], line["line_label"]
                )
        ...
        if checkpoint_metrics:
            ...
            validator_eval.add_event_handler(
                ieng.Events.EPOCH_COMPLETED, model_saver, {model_name: model}
            )
        ...
        def log_training(engine: ieng.Engine):
            ...
            if (
                num_iter % log_interval == 0
                or num_iter == len(train_loader)
                or early_stop
            ):
                tqdm.tqdm.write(
            ...
            if early_stop:
                tqdm.tqdm.write(
                    colored("Early stopping due to invalid loss value.", "red")
                )
                trainer.terminate()
                ...
                for line in scope["lines"]:
                    if line["update_rate"] == "epoch":
                        try:
                            ...
                            tqdm_info.append(
                                "{}: {:.4f}".format(line_label, line_value)
                            )
                        except KeyError:
                            pass
...
def main():
    with _utils.tqdm_stdout() as orig_stdout:
        ...
        parser.add_argument(
            "-S", "--skip-train-val", action="store_true", default=False
        )
        ...
        for config_path in prog_bar_exps:
                ...
                if arg.startswith("--"):
                    ...
                    if found:
                        ...
                        override_parser.add_argument(
                            arg, nargs=section_nargs, type=section_type
                        )
                        overridden_args, _ = override_parser.parse_known_args(
                            unknown_args
                        )
                        ...
                        tqdm.tqdm.write(
                            colored(
                                f'Overriding "{overridden_key}": {old_value} -> {overriding_value}',
                                "magenta",
                            )
                        )
                    else:
            ...
            saved_models_path = _utils.arg_selector(
                ...
                config["Setup"]["saved_models_path"],
                SAVED_MODELS_PATH,
            )
