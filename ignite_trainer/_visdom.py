...
def calc_ytick_range(
    vis: visdom.Visdom, window_name: str, env: Optional[str] = None
) -> Tuple[float, float]:
    ...
    if stats:
        ...
        if stats:
            q25s = np.array(
                [np.quantile(item, 0.25) for item in stats if len(item) > 0]
            )
            q75s = np.array(
                [np.quantile(item, 0.75) for item in stats if len(item) > 0]
            )
            ...
            if q25s.shape == q75s.shape and len(q25s) > 0:
                ...
                for item in stats:
                    if len(item) > 0:
                        item_sanitized = item[
                            (item >= lower_bounds[idx]) & (item <= upper_bounds[idx])
                        ]
                ...
                q25_sanitized = np.array(
                    [np.quantile(item, 0.25) for item in stats_sanitized]
                )
                q75_sanitized = np.array(
                    [np.quantile(item, 0.75) for item in stats_sanitized]
                )
...
def plot_line(
    ...
    window_name = vis.line(
        X=x, Y=y, win=window_name, env=env, update="append", name=line_label, opts=opts
    )
...
def create_summary_window(
    vis: visdom.Visdom, visdom_env_name: str, experiment_name: str, summary: str
) -> str:
...
def get_visdom_instance(
    host: str = "localhost",
    port: int = 8097,
    env_name: str = "main",
    env_path: str = "visdom_env",
) -> Tuple[visdom.Visdom, Optional[int]]:
    ...
    if not connection_is_alive(host, port):
        if any(host.strip("/").endswith(lh) for lh in ["127.0.0.1", "localhost"]):
            ...
            vis_proc = subprocess.Popen(
                vis_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            time.sleep(2.0)
    ...
    while not connection_is_alive(host, port):
        ...
        if trials_left < 1:
            raise RuntimeError(
                'Visdom server is not running. Please run "python -m visdom.server".'
            )

    vis = visdom.Visdom(server="http://{}".format(host), port=port, env=env_name)
