...
class Wavlm_lit(DeepfakeAudioClassification):
    ...
    def _shared_pred(self, batch, batch_idx):
        ...
        wav1 = self.model.compute_stage1(audio)
        wav_feat, raw_wav_feat = self.model.compute_rest_stage(wav1)
        batch_out = self.norm_feat(wav_feat)
        batch_out = self.cls(batch_out).squeeze(-1)
    ...
    def forward(self, audio):
        ...
        wav_feat, raw_wav_feat = self.model.compute_rest_stage(wav1)
        batch_out = self.norm_feat(wav_feat)
        batch_pred = self.cls(batch_out).squeeze(-1)
        batch_pred = (torch.sigmoid(batch_pred) + 0.5).int()
        return {"logit": batch_out, "pred": batch_pred,"final_feat":batch_out}
