...
class WavLM_1D(nn.Module):
    def __init__(
        ...
        self.model1D = WavLM()
        self.n_dim = 768
        self.post_extractor = post_process(hidden_dim=12544)

    # def forward(self, x):
    #     if x.ndim == 3:
    #         x = x[:, 0, :]
    #     feature = self.model1D.pretrain_model(x)[
    #         self.model1D.pretrain_feat
    #     ]  # (B, T, 768)
    #     return feature.mean(1)
    def forward(self,x,target_layer):
        hidden_states=self.compute_stage1(x)
        ...
        position_bias = None
        hidden_states, position_bias = self.compute_transformer_layers(
            hidden_states, 0, 3, position_bias=None
        )
        if len(target_layer)!=1:
            hidden_states0,_=self.compute_transformer_layers(
                hidden_states, 3, target_layer[0], position_bias=position_bias
            )
            hidden_states1,_=self.compute_transformer_layers(
                hidden_states0, target_layer[0], target_layer[1], position_bias=position_bias
            )
            multi_scale_features = torch.cat([hidden_states0,hidden_states1], dim=-1)
            # hidden_states, position_bias = self.compute_transformer_layers(
            #     hidden_states, 3, target_layer, position_bias=position_bias
            # )
    ...
    def compute_rest_stage(self, x):
        position_embeddings = self.model1D.pretrain_model.encoder.pos_conv_embed(x)
        ...
        hidden_states, position_bias = self.compute_transformer_layers(
            hidden_states, 3, 12, position_bias=position_bias
        )
        # x = self.post_extractor(hidden_states)
    ...
    def compute_transformer_layers(self, hidden_states, s, e, position_bias=None):
        ...
        for i in range(s, e):
            ...
            skip_the_layer = (
                ...
                and i > 0
                and (
                    dropout_probability < self.model1D.pretrain_model.encoder.layerdrop
                )
            )
    ...
    def compute_transformer_layers_adapter(self, hidden_states, s, e, adapters,position_bias=None):
        # if self.position_bias is not None:
        # print(self.position_bias.shape)
        for i in range(s, e):
            layer = self.model1D.pretrain_model.encoder.layers[i]
    ...
    def compute_stage2(self, hidden_states):
        position_embeddings = self.model1D.pretrain_model.encoder.pos_conv_embed(
            hidden_states
        )
        hidden_states = hidden_states + position_embeddings
        hidden_states = self.model1D.pretrain_model.encoder.layer_norm(hidden_states)
...
class CrossAttention2D(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_channels=time_dim, out_channels=feature_dim, kernel_size=1
        )
        self.conv2 = nn.Conv2d(
            in_channels=spec_dim, out_channels=feature_dim, kernel_size=1
        )
    ...
    def forward(self, waveform, spectrogram):
        ...
        attn_weights = self.softmax(
            torch.matmul(query, key.transpose(-2, -1)) / (self.feature_dim**0.5)
        )
...
class Expand(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv1d(
            in_channels=time_len, out_channels=spec_height * spec_width, kernel_size=1
        )
        self.conv2 = nn.Conv2d(
            in_channels=time_dim, out_channels=spec_dim, kernel_size=3, padding=1
        )

        self.layer_norm1 = nn.LayerNorm(time_dim)
        self.layer_norm2 = nn.LayerNorm(spec_dim)
        self.attn = CrossAttention2D(
            time_dim=time_dim, spec_dim=spec_dim, feature_dim=spec_dim
        )
    ...
    def forward(self, x, y):
        ...
        if self.use_PE:
            norm_x = (
                self.compute_layernorm(x, self.layer_norm1)
                + self.positional_encoding_wave
            )
            norm_y = (
                self.compute_layernorm(y, self.layer_norm2)
                + self.positional_encoding_spec
            )
        else:
...
class MultiHeadCrossAttention1D(nn.Module):
    ...
    def forward(self, waveform, spectrogram):
        ...
        keys = keys.view(batch_size, -1, self.num_heads, self.feature_dim_head).permute(
            0, 2, 1, 3
        )
        queries = queries.view(
            batch_size, -1, self.num_heads, self.feature_dim_head
        ).permute(0, 2, 1, 3)
        values = values.view(
            batch_size, -1, self.num_heads, self.feature_dim_head
        ).permute(0, 2, 1, 3)
        ...
        score = torch.matmul(queries, keys.transpose(-2, -1)) / math.sqrt(
            self.feature_dim_head
        )
...
class Squeeze(nn.Module):
    def __init__(
        ...
        self.positional_encoding_wave = nn.Parameter(
            torch.randn(1, time_len, time_dim), requires_grad=True
        )
        self.positional_encoding_spec = nn.Parameter(
            torch.randn(1, time_len, spec_dim), requires_grad=True
        )
        self.use_PE = use_PE
        self.drop_layer = drop_layer
        self.use_fusion = use_fusion
...
class WaveformToSpectrogram(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(
            149, 257, kernel_size=3, stride=1, padding=1
        )  # Adjust the channels
...
class GatedFusionLayer(nn.Module):
    ...
    def forward(self, waveform_features, spectrogram_features):
        ...
        spectrogram_features_flat = spectrogram_features.view(
            spectrogram_features.size(0), -1
        )

        # Concatenate features
        combined_features = torch.cat(
            [waveform_features_flat, spectrogram_features_flat], dim=-1
        )
