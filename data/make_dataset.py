...
def get_InTheWild_data(
    root_path="/home/zyz/data/in_the_wild",
):
...
def get_WaveFake_JP(root_path="/home/ay/data/DATA/2-datasets/1-df-audio/WaveFake"):
    ...
    train, val = dataset.split_data(
        data_train, splits=[0.8, 0.2], return_list=True, refer="id"
    )
...
def get_ASV2019_test_split(root_path="/home/zyz/data/LA_2019"):
...
def get_ASV5_test_splits(root_path="/home/zyz/data/ASVSpoof5/ASVSpoof5"):
...
# +
def get_ASV2021_test_splits(root_path="/home/zyz/data/LA_2021/ASVspoof2021_LA_eval"):
    dataset = ASV2021_AudioDs(root_path=root_path)
    data_splits = dataset.get_test_splits()
    return data_splits


def get_ASV2021_whole_test_split(root_path="/home/zyz/data/LA_2021/ASVspoof2021_LA_eval"):
    dataset = ASV2021_AudioDs(root_path=root_path)
    test = dataset.get_whole_test_split()
    return [test]

def get_ASV2021_DF_test_split(root_path="/home/zyz/data/2021DF/ASVspoof2021_DF_eval"):
    dataset = ASV2021_AudioDs(root_path=root_path)
    test = [dataset.get_whole_test_split()]
...
def make_MLAAD(cfg):
    ...
    if cfg.task == "cross_lang":
        ...
        data_splits.train = pd.concat(
            [en_splits.train, ch_splits.train, jp_splits.train], ignore_index=True
        )
        data_splits.val = pd.concat(
            [en_splits.val, ch_splits.val, jp_splits.val], ignore_index=True
        )
    return data_splits
