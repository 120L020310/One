...
from test import choose_noisy, get_augment, get_wepe

class ALDA_OneClass_AugImmunity_wepe_Lit(DeepfakeAudioClassification):
    def __init__(self, cfg=None, args=None, proj=False, **kwargs):
        ...
        # self.augmentor = RobustDataAugmentor(p=1.0)
        self.augmentor1 = SafeRawAugmentor(noise_intensity=0.1, mask_ratio=0.1)
        # self.augmentor2 = TransformerHardAugmentor(mask_ratio=0.1, span_len=4800)
        # self.augmentor = DigitalArtifactAugmentor()
    ...
    def _shared_pred(self, batch, batch_idx, stage='train', **kwargs):
        ...
        if self.training:
            feat_noisy = choose_noisy(self,audio)
        else:
            feat_noisy = embedding
