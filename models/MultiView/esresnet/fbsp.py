...
from models.MultiView.esresnet import transforms
from models.MultiView.esresnet.base import _ESResNet
from models.MultiView.esresnet.base import Bottleneck
...
class LinearFBSP(torch.nn.Module):
    ...
    def __init__(self, out_features: int, bias: bool = True, normalized: bool = False):
        ...
        self.register_parameter(
            "m", torch.nn.Parameter(torch.zeros(self.out_features, dtype=default_dtype))
        )
        self.register_parameter(
            "fb", torch.nn.Parameter(torch.ones(self.out_features, dtype=default_dtype))
        )
        self.register_parameter(
            "fc",
            torch.nn.Parameter(torch.arange(self.out_features, dtype=default_dtype)),
        )
        self.register_parameter(
            "bias",
            torch.nn.Parameter(
                torch.normal(0.0, 0.5, (self.out_features, 2), dtype=default_dtype)
                if bias
                else cast(torch.nn.Parameter, None)
            ),
        )

        self.m.register_hook(
            lambda grad: grad / (torch.norm(grad, p=float("inf")) + self.eps)
        )
        self.fb.register_hook(
            lambda grad: grad / (torch.norm(grad, p=float("inf")) + self.eps)
        )
        self.fc.register_hook(
            lambda grad: grad / (torch.norm(grad, p=float("inf")) + self.eps)
        )
    ...
    def sinc(x: torch.Tensor) -> torch.Tensor:
        return torch.where(
            cast(torch.Tensor, x == 0), torch.ones_like(x), torch.sin(x) / x
        )
    ...
    def _materialize_weights(self, x: torch.Tensor) -> Tuple[torch.Tensor, bool]:
        ...
        t = (
            np.pi
            * torch.linspace(
                -1.0, 1.0, in_features, dtype=x.dtype, device=x.device
            ).reshape(1, -1, 1)
            + self.eps
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if x_is_complex:
            x = torch.stack(
                (
                    F.linear(x[..., 0], weights[..., 0])
                    - F.linear(x[..., 1], weights[..., 1]),
                    F.linear(x[..., 0], weights[..., 1])
                    + F.linear(x[..., 1], weights[..., 0]),
                ),
                dim=-1,
            )
        else:
            x = torch.stack(
                (F.linear(x, weights[..., 0]), F.linear(x, weights[..., 1])), dim=-1
            )
...
class _ESResNetFBSP(_ESResNet):
    ...
    def _inject_members(self):
        self.add_module(
            ...
            LinearFBSP(
                out_features=(
                    int(round(self.n_fft / 2)) + 1 if self.onesided else self.n_fft
                ),
