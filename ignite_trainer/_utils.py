...
def arg_selector(
    arg_cmd: Optional[Any], arg_conf: Optional[Any], arg_const: Any
) -> Any:
...
def get_data_loaders(
    ...
    dataset_mode_train = {
        dataset_args["training"]["key"]: dataset_args["training"]["yes"]
    }
    dataset_mode_test = {
        dataset_args["training"]["key"]: dataset_args["training"]["no"]
    }

    dataset_args_train = {
        **{k: v for k, v in dataset_args.items() if k != "training"},
        **dataset_mode_train,
    }
    dataset_args_test = {
        **{k: v for k, v in dataset_args.items() if k != "training"},
...
def build_summary_str(
    ...
    summary_window_text += (
        '<a style="cursor: pointer;" onclick="jQuery(\'#{}\').toggle()">'.format(
            setup_title
        )
    )
    ...
    summary_window_text += "</h3>"
    summary_window_text += (
        '<div style="margin: 5px; padding: 5px; background-color: lightgray;">'
    )
    summary_window_text += '<div id="{}" style="display: none;"><pre>'.format(
        setup_title
    )
            ...
            "Transforms": OrderedDict(
                {
                    "Training": OrderedDict(
                        {tr["class"]: tr["args"] for tr in transforms if tr["train"]}
                    ),
                    "Validation": OrderedDict(
                        {tr["class"]: tr["args"] for tr in transforms if tr["test"]}
                    ),
                }
            ),
        }
    ...
    summary_window_text += "{}".format(json.dumps(summary, indent=2))
