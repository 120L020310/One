...
class VGGSound_AudioDs(AudioDataset):
    def postprocess(self):
        self.data["audio_path"] = self.data["filename"].apply(
            lambda x: os.path.join(self.root_path, "audio", x)
        )
    ...
    def _read_metadata(self, root_path, *args, **kwargs):
        ...
        data["filename"] = data.apply(
            lambda x: f"{x['id']}_{'%06d'%(x['start_second'])}.wav", axis=1
        )
        data["audio_path"] = data["filename"].apply(
            lambda x: os.path.join(root_path, "audio", x)
        )
    ...
    def split_data(self, splits=None):
        ...
        return Namespace(train=train, test=test)
