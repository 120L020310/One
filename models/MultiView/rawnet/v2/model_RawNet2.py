...
class Residual_block_wFRM(nn.Module):
    def __init__(self, nb_filts, first=False):
        ...
        self.frm = FRM(nb_dim=nb_filts[1], do_add=True, do_mul=True)
...
class SincConv_fast(nn.Module):
    ...
    def forward(self, waveforms):
        ...
        # print(self.filters.shape, self.stride, waveforms.shape)
...
class RawNet2(nn.Module):
    def __init__(self, d_args):
        ...
        self.block0 = nn.Sequential(
            Residual_block_wFRM(nb_filts=d_args["filts"][1], first=True)
        )
        self.block1 = nn.Sequential(
            Residual_block_wFRM(nb_filts=[d_args["filts"][1][1]] * 2)
        )
        ...
        self.block4 = nn.Sequential(Residual_block_wFRM(nb_filts=d_args["filts"][3]))
        self.block5 = nn.Sequential(
            Residual_block_wFRM(nb_filts=[d_args["filts"][3][1]] * 2)
        )
    ...
    def get_content_stem(self):
        return [self.gru, self.fc1_gru]

    def get_vocoder_stem(self):
        return [self.gru_copy, self.fc1_gru_copy]
