...
def read_metadata_of_MLADD(root_path):
    ...
    real_data["audio_path"] = real_data["path"].apply(
        lambda x: os.path.join(real_folder, x)
    )
    ...
    fake_data = data.copy()
    fake_data["label"] = 0
    fake_data["audio_path"] = fake_data["path"].apply(
        lambda x: os.path.join(fake_folder, x[7:])
    )
...
class MLAAD_AudioDs(AudioDataset):
    ...
    def postprocess(self):
        ...
        self.data["audio_path"] = self.data["path"].apply(
            lambda x: (
                os.path.join(fake_folder, x[7:])
                if x.startswith("./fake")
    ...
    def get_splits(self, language_list=["en", "de", "es"]):
        ...
        in_domain_data = self.data.query(f"language in {language_list}").reset_index(
            drop=True
        )
        if len(in_domain_data) > 10:
            train, val, in_domain_test = self.split_data(
                in_domain_data, splits=[0.8, 0.1, 0.1], return_list=True
            )
        else:
            train, val, in_domain_test = None, None, None

        # out_domain_data = self.data.query(
        #     f"language not in {language_list}"
        # ).reset_index(drop=True)
        out_domain_lang_list=["fr","it","pl","ru","uk"]
        out_domain_data = self.data.query(
            f"language in {out_domain_lang_list}"
        ).reset_index(drop=True)
    ...
    def get_true_train(self, data=None):
        ...
        if len(in_domain_data) > 10:
            train_data, val, in_domain_test = self.split_data(
                in_domain_data, splits=[0.8, 0.1, 0.1], return_list=True
            )
        else:
            train, val, in_domain_test = None, None, None
