...
def logistic_map(x, num):
    """use to Logistic Map to generate random numbers

    Logistic Map: F(x) = r * x * (1 - x), r is set to 4 as default

    Args:
        x: the initial value of x
        num: the number of generated random numbers
    """
    assert x > 0 and x < 1
    X = []
    for i in range(num):
