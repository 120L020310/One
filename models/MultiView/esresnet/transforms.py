...
def frame_signal(
    ...
    if window.shape[0] != frame_length:
        raise ValueError(
            "Wrong `window` length: expected {}, got {}".format(
                window.shape[0], frame_length
            )
        )
    ...
    if signal_length <= frame_length:
        ...
        num_frames = 1 + int(
            math.ceil((1.0 * signal_length - frame_length) / hop_length)
        )

    pad_len = int((num_frames - 1) * hop_length + frame_length)
    if pad_len > signal_length:
        zeros = torch.zeros(
            pad_len - signal_length, device=signal.device, dtype=signal.dtype
        )

        while zeros.dim() < signal.dim():
            zeros.unsqueeze_(0)

        pad_signal = torch.cat(
            (zeros.expand(*signal.shape[:-1], -1)[..., : zeros.shape[-1] // 2], signal),
            dim=-1,
        )
        pad_signal = torch.cat(
            (
                pad_signal,
                zeros.expand(*signal.shape[:-1], -1)[..., zeros.shape[-1] // 2 :],
            ),
            dim=-1,
        )
    else:
